...
## Mass Lead Converter Install Instructions

1. This application is for Mass Converting Leads.
2. After installation of the application,
   add the "Mass Convert Leads" button to the Lead List View layout.
...
3. Clone the MassLeadConverter repository:

    ```
    git clone https://github.com/salesforcelabs/Mass-Lead-Converter
    cd Mass-Lead-Converter
    ```

4. Create a scratch org and provide it with an alias (**MassLeadConverter** in the command below):

    ```
    sfdx force:org:create -s -f config/project-scratch-def.json -a MassLeadConverter
    ```
...
6. Assign the `MassLeadConverter` permission set to the default user.

    ```
    sfdx force:user:permset:assign -n MassLeadConverter
    ```
...
8. In App Launcher, click **View All** then select the **MassLeadConverter** app.
...
7. In App Launcher, select the **MassLeadConverter** app.
